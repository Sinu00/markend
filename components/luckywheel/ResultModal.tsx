...
import type { SpinWheelResult } from "@/lib/luckywheel/types";
...
export default function ResultModal({ open, result, confettiEnabled, claimExpiresAt, onClose, onClaim }: Props) {
  ...
  return (
    <AnimatePresence>
      {open && result && (
        <motion.div
          className="fixed inset-0 z-[200] grid place-items-center bg-black/85 p-4"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        >
          <motion.div
            ...
            className={`relative w-full max-w-lg rounded-3xl border-2 p-8 text-center ${isWin ? "border-[#6ed807] bg-[#181818]" : "border-[#333] bg-[#181818]"}`}
          >
            <Confetti active={isWin && confettiEnabled} />
            <p className="text-6xl">{result.emoji ?? (isWin ? "🎉" : "😢")}</p>
            <h3
              className={`mt-4 text-4xl ${isWin ? "text-[#6ed807]" : "text-white"}`}
            ...
            <p className="mt-3 text-sm text-[#aaa]">
              {isWin
                ? "Show this screen to our team to claim your prize."
                : "Follow us on Instagram for more giveaways: @markend.in"}
            </p>
            ...
            {isWin && (
              <div className="mt-5">
                ...
                <div className="mt-2 h-2 w-full rounded-full bg-[#2a2a2a]">
                  <div
                    className="h-2 rounded-full bg-[#6ed807] transition-all"
            ...
            <div className="mt-6 flex justify-center gap-3">
              {isWin ? (
                <button
                  type="button"
                  onClick={onClaim}
                  className="rounded-full bg-[#6ed807] px-8 py-3 font-bold text-[#181818]"
                >
                  Claim now
                </button>
              ) : (
                <button
                  type="button"
                  onClick={onClose}
                  className="rounded-full border border-[#333] px-8 py-3 text-white"
                >
                  Close
                </button>
              )}
